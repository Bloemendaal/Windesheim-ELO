...
(function() {
   ...
   function initELO() {
      ...
      upload = UIkit.upload('#handin-upload', {
         ...
         loadStart: function (e) {
            ...
            progressStatus.text(getUnit(e.loaded) + " / " + getUnit(e.total)).slideDown(250);
         },
         progress: function (e) {
            progressBar.css('transform', 'scaleX('+(e.loaded/e.total).toFixed(2)+')');
            progressStatus.text(getUnit(e.loaded) + " / " + getUnit(e.total));
         },
         loadEnd: function (e) {
            progressBar.css('transform', 'scaleX(1)');
         },
         completeAll: function() {
            progressStatus.slideUp(250);
            prepareHandin($('#container-handin').data('resource'));
         }
      });
   ...
   function getUnit(bytes) {
      if (bytes / 1E9 > 1) {
         return (Math.round(bytes / 1E7) / 1E2) + " GB";
      } else if (bytes / 1E6 > 1) {
         return (Math.round(bytes / 1E4) / 1E2) + " MB";
      } else {
         return (Math.round(bytes / 1E2) / 1E1) + " kB";
      }
   }
